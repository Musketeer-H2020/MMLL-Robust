...
class NN_Worker(POM1_CommonML_Worker):
    ...
    def ProcessReceivedPacket_Worker(self, packet):
        ...
        if packet['action'] == 'INIT_MODEL':
            ...
            """
            gradients = K.gradients(self.model.keras_model.total_loss, self.model.keras_model.trainable_weights)
            inputs = self.model.keras_model._feed_inputs + self.model.keras_model._feed_targets + self.model.keras_model._feed_sample_weights
            self.get_gradients = K.function(inputs, gradients)
    ...
    def get_weight_grad(self, model_weights, num_data=None):
        ...
        x, y, sample_weight = self.model.keras_model._standardize_user_data(x_batch, y_batch)
        # Get gradients and loss
        output_grad = self.get_gradients(x + y + sample_weight)
