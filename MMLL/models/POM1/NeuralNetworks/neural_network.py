...
class NN_Master(POM1_CommonML_Master):
    ...
    def __init__(self, comms, logger, verbose=False, model_architecture=None, Nmaxiter=10, learning_rate=0.0001, momentum=0, nesterov=False, model_averaging='True', optimizer='adam', loss='categorical_crossentropy', metric='accuracy', batch_size=32, num_epochs=1, Tmax=None, aggregator = None):
        """
    ...
    def TakeAction_Master(self):
        ...
        if self.state_dict['CN'] == 'UPDATE_MODEL':
            self.aggregator.aggregate(self.model, self.dict_gradients)
            self.reset()
        ...
        if self.state_dict['CN'] == 'MODEL_AVERAGING':
            self.aggregator.aggregate(self.model, self.dict_weights)
            self.reset()
            self.state_dict['CN'] = 'CHECK_TERMINATION'
            self.iter += 1
