...
Jan. 2020
python pom6_KR_pm_worker_localflask.py --id 0 --dataset sinc1D --verbose 1
...
if __name__ == "__main__":
    ...
    display('Creating WorkerNode under POM6, communicating through localflask', logger, True)
    #########################################
    ...
    #########################################
    # Creating Comms object, needed by MMLL
    comms = Comms(my_id=str(int(worker_address) + 5))
    #########################################
    # Creating Workernode
    wn = WorkerNode(pom, comms, logger, verbose=verbose, master_address='ma2')
    display('-------------------- Loading dataset %s --------------------------' % dataset_name, logger, True)
    # Warning: this data connector is only designed for the demos. In Musketeer, appropriate data
    # connectors must be provided
    data_file = '../../../../input_data/' + dataset_name + '_demonstrator_data.pkl'
