...
if __name__ == "__main__":
    ...
    ytr = np.array(ytr).astype(np.float)
    ...
    output_filename_model = './results/models/Worker_' + dataset_name + '_model'
    ...
    y = np.array(ytst).astype(np.float)
    classes = np.arange(2) # 0 to 9

    display('-------------  Evaluating --------------------------------------------\n', logger, verbose)
    # Warning, these evaluation methods are not part of the MMLL library, they are only intended
    # to be used for the demos. Use them at your own risk.
    output_filename = 'Worker_' + worker_address + '_NN_confusion_matrix_' + dataset_name + '.png'
